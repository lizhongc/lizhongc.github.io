...
"""jemdoc version 0.7.3, 2012-11-27. (Python 3 port)"""
...
class strfile(object):
  """In-memory file wrapper backed by a string with an integer cursor.

  Provides exactly the operations jemdoc's parser needs (read, readline,
  seek, close). Because it is backed by a string, the peek/rewind pattern in
  pc() -- read(1) followed by seek(-1, 1) -- works unchanged under Python 3,
  where real text streams forbid nonzero current-relative seeks."""
...
def openfile(name):
  """Open a source/conf/menu file and return a seekable string wrapper."""
...
def parseconf(cns):
  ...
  fs = [strfile(standardconf())]
  for sname in cns:
    fs.append(openfile(sname))
...
def doincludes(f, l):
  ...
  i = 'include{'
  if l.startswith(ir):
    with open(l[len(ir):-2], encoding='utf-8') as nf:
...
def replaceequations(b, f):
  ...
  for (r, wl) in rs:
    ...
    while m:
      eq = m.group(1)
      if wl:
        fn = str(int(hashlib.md5((eq + 'wl120930alsdk').encode('utf-8')).hexdigest(), 16))
      else:
        fn = str(int(hashlib.md5(eq.encode('utf-8')).hexdigest(), 16))
...
def geneq(f, eq, dpi, wl, outname):
  ...
  try:
    ...
    p = Popen(latexcmd, shell=True, stdout=PIPE, universal_newlines=True)
    rc = p.wait()
    if rc != 0:
